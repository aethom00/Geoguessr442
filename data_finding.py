...
def generate_images(gui, app_access_token, limit, num_images=5, verbose=False, image_cache=set()):
    ...
    while images_downloaded < num_images:
        ...
        image_ids = find_images_in_bbox(bbox, app_access_token, limit=limit)
        image_ids = [image_id for image_id in image_ids if image_id not in image_cache]  # Remove already downloaded images
        num_images_to_download = min(num_images - images_downloaded, len(image_ids))
        if num_images_to_download > 0:
            for image_id in image_ids[:num_images_to_download]:
                image_url, image_coordinates = get_image_url_from_id(image_id, app_access_token)
            ...
            if verbose:
                print(f"No new images found in the bounding box: [({top_left[1]}, {bottom_right[0]}), ({bottom_right[1]}, {top_left[0]})].")
